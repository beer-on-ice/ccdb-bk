<template>
	<div class="policyManagementWrapper">
		<div class="popWrapper">
			<WarrantyLandEdit ref="warrantyEdit"
				:visible="warrantyEditVisible"
				@viewPdfOrImg="viewPdfOrImg"
				@handleWarrantyEditClose="handleWarrentyClose"></WarrantyLandEdit>
			<WarrantyLandAdd :visible="warrantyAddVisible"
				@viewPdfOrImg="viewPdfOrImg"
				@handleWarrantyAddClose="handleWarrentyClose"></WarrantyLandAdd>
			<a href=""
				ref="a"
				target="_blank"
				style="display:none">
					...
					<a-row>
						<a-col class="slideShowItem"
							...
							<p class="clearfix">
								...
								<a-button class="btn"
									type="primary"
									@click="handleSlideShowEdit(item.id, 'slideLand', true)">{{ item.state ? "已发布" : "待发布" }}</a-button>
						...
						<a-col class="slideShowItem slideShowItemS"
							...
							<p>
								<a-button class="btn"
									@click="handleSlideShowAdd('slideLand', true)">新增</a-button>
							</p>
						</a-col>
					</a-row>
				</div>
			</div>
			<first-table-list-land @handleWarrantyEdit="handleWarrantyEdit"
				@handleWrrantyAdd="handleWrrantyAdd"
				ref="firstTable"></first-table-list-land>
...
export default {
  ...
  methods: {
    ...
    // 轮播 - 获取
    getNewestPolicy () {
      getListInsuranceBanner({ serviceType: 6 }).then(res => {
