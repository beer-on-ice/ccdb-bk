...
export default {
  ...
  data () {
    return {
      ...
      loadData: () => {
        return multipleCompanyHonors(this.queryParam).then(res => {
  ...
  created () {
    ...
    this.showCompanyHonorsCount()
  },
  methods: {
    async showCompanyHonorsCount () {
      let res = await showCompanyHonorsCount()
      if (res.code === 200) {
